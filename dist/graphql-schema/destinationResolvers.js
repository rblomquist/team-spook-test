...
export const destinationResolvers = {
    Query: {
        ...
        getDestinationbyId: async (_, args) => {
            try {
    ...
    Mutation: {
        createDestination: async (_, args) => {
        ...
        updateDestination: async (_, args) => {
            const sanitizedArgs = sanitize(args);
            const { id, country, city, currency, language, description, meals, lodging, price, Bus_id, image } = sanitizedArgs;
            return await Destination.findByIdAndUpdate(id, { $set: { id, country, city, currency, language, description, meals, lodging, price, Bus_id, image } }, { new: true });
        },
        deleteDestination: async (_, args) => {
            try {
                const destinationDeleted = await Destination.findByIdAndDelete(args.id);
                const Answer = destinationDeleted && `Destination deleted succesfully : ${destinationDeleted}`;
