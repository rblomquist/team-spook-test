...
export const agentResolvers = {
    ...
    Mutation: {
        ...
        updateAgent: async (_, args) => {
            const sanitizedArgs = sanitize(args);
            const { id, email, password, firstName, lastName, phone, databaseAccessLevel } = sanitizedArgs;
            return await Agent.findByIdAndUpdate(id, { $set: { email, password, firstName, lastName, phone, databaseAccessLevel } }, { new: true });
        },
        deleteAgent: async (_, args) => {
            try {
                const agentDeleted = await Agent.findByIdAndDelete(args.id);
                const Answer = agentDeleted && `Agent deleted succesfully : ${agentDeleted}`;
